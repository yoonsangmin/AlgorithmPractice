...
std::vector<Node*> AStarOld::FindPath(Node* startNode, Node* goalNode, const std::vector<std::vector<int>>& grid)
{
...
std::vector<Node*> AStarOld::ConstructPath(Node* goalNode)
{
...
float AStarOld::CalculateHeuristic(Node* currentNode, Node* goalNode)
{
...
bool AStarOld::IsInRange(int x, int y, const std::vector<std::vector<int>>& grid)
{
...
bool AStarOld::HasVisited(int x, int y, float gCost)
{
...
bool AStarOld::IsDestination(Node* node)
{
	// 노드의 위치가 서로 같은지 비교.
	return *node == *goalNode;
}

void AStarOld::DisplayGridWithPath(std::vector<std::vector<int>>& grid, const std::vector<Node*>& path)
{
